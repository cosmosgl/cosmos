...
Images are rendered above shapes, so if you explicitly set shapes to values other than `None` (8), both shapes and images will be visible (shapes underneath, images on top).
...
**Note:** When images are set, the default point shapes automatically change to `None` (8) to avoid visual clutter. Images are rendered above shapes, so if you explicitly set shapes to values other than `None` (8), both shapes and images will be visible (shapes underneath, images on top).
...
// To show shapes with images (shapes underneath, images on top)
